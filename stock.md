...
## Twelve Data 연동 이후 남은 작업 로드맵

Twelve Data를 통해 일별 시세 적재까지 확인했으므로, 다음 단계에서 구현해야 할 주요 업무를 우선순위 순으로 정리합니다.

1. **뉴스/감성 데이터 파이프라인 보강**
   - Finnhub·NewsData.io WebClient 구현, 수집 스케줄러 구성, `stock_news_cache` 적재 및 TTL/정합성 검증.
   - 감성 점수 산식/임계값 정의, 실패 재시도와 Rate Limit 대응 로직 추가.

2. **스캔 프로필·필터 CRUD + 문서화**
   - `stock.v1`에 Handler/Service/Repository/DTO를 구성하고 WebFlux 라우팅을 노출.
   - 입력 Validation, MapStruct 매핑, @WebFluxTest + REST Docs로 계약 자동화.

3. **스캔 엔진 및 시그널 생성기**
   - 필터 표현식 DSL·전략 패턴 도입, SQL/인메모리 평가를 선택적으로 수행.
   - 기술·뉴스·리스크 점수 산식 최종 확정, 시그널 중복 방지와 upsert 처리.

4. **알림 파이프라인**
   - 알림 대상 선정 규칙, 채널 추상화(NotificationSender), 중복 방지·throttle 구현.
   - `notification_log` 상태 머신(PENDING→SUCCESS/FAILED)과 재시도/Dead Letter 처리.

5. **대시보드·리포트 API & 프론트**
   - 시그널 집계·상세·히스토리 API, React 대시보드(랭킹, 차트, 뉴스 타임라인) 구성.
   - SSE/WebSocket으로 실시간 알림을 확장할 수 있는 Hook 준비.

6. **운영/모니터링**
   - Prometheus 지표(수집/스캔/알림 성공/실패), Grafana 대시보드, 장애 알림 채널 연동.
   - 데이터 품질 점검 배치(결측치, 비정상 변동) 및 보정 도구.

7. **테스트·배포 자동화**
   - Testcontainers 기반 통합 테스트, Twelve Data MockServer, 배치 E2E 시나리오.
   - Gradle CI 파이프라인, Docker 이미지화, Stage/Prod 배포 전략 수립.

로드맵을 순차적으로 수행하면 시세 수집 → 뉴스 분석 → 스캔/시그널 → 알림 → 대시보드까지 일관된 파이프라인을 완성할 수 있습니다. 각 단계는 이전 산출물을 기반으로 하므로 모니터링·리트라이 체계를 병행 구축해야 안정적으로 운영할 수 있습니다.
